...
async def get_team_member(id: int, current_user: dict = Depends(get_current_user)):
    res = supabase_admin_client.table("team_members").select("*").eq("id", id).execute()
    if not res.data:
...
async def create_team_member(
    ...
    # 5. Insert to DB
    try:
        db_res = supabase_admin_client.table("team_members").insert({
            "name": name,
            "role": role,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to save team member to database: {str(e)}")
...
async def update_team_member(
    ...
    try:
        if update_data:
            db_res = supabase_admin_client.table("team_members").update(update_data).eq("id", id).execute()
            if not db_res.data:
        ...
        if file is not None and old_image_path:
            try:
                ...
                if "/team-images/" in old_image_path:
...
@router.delete("/{id}")
async def delete_team_member(id: int, current_user: dict = Depends(require_console_admin)):
    res = supabase_admin_client.table("team_members").select("*").eq("id", id).execute()
    if not res.data:
        raise HTTPException(status_code=404, detail="Team member not found")
    record = res.data[0]
    ...
    bucket = "team-images"
    if image_path and "/team-images/" in image_path:
        try:
    ...
    try:
        db_res = supabase_admin_client.table("team_members").delete().eq("id", id).execute()
